...
public class QryParser {
    ...
    private static Qry createOperator(String operatorName) {
        ...
        //  #near/n and #window/n.
        String[] params = operatorNameLowerCase.split("/");

        if (params.length > 1) {
            operatorDistance = Integer.parseInt(params[1]);
            if (params[0].equals("#near")) {
                operatorNameLowerCase = "#near";
            }
            if (params[0].equals("#window")) {
                operatorNameLowerCase = "#window";
            }
        }
    ...
    private static Qry parseString(String queryString)
        ...
        while (queryString.length() > 0) {
            ...
            //  System.out.println(queryString);//for shuning's testing

            char firstArg = queryString.charAt(0);//for shuning's testing
            boolean isWeight = Character.isDigit(firstArg);
            StringBuilder weight = new StringBuilder();
            Double weightArg = 0.0;
            if (Character.isDigit(firstArg)) {

                for (Character c : queryString.toCharArray()) {
                    if (c != ' ') {
                        weight.append(Character.toString(c));
                    } else {
                        break;
                    }
                }
                ...
                try {
                    queryString = queryString.substring(weight.toString().length() + 1);//queryString 0.1 Green //0,1,2,3,4
                    weightArg = Double.parseDouble(weight.toString());
                   // weights.add(weightArg);
        ...
        if (queryTree instanceof QrySopWAnd) {
            ((QrySopWAnd) queryTree).setWeights(weights);
        }
        if (queryTree instanceof QrySopWSum) {
            ((QrySopWSum) queryTree).setWeights(weights);
        }
